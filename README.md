...
- 后来遇到了这个应用数据丢失的问题，且无法恢复，历史记录不存在。然而md格式不太美观，于是新版本里添加html方便为迁移到平台提供原版的画面。
- wolai上，某个页面约有2k个文档，耗时md格式9分钟+，html格式3分钟+
- 由于官方目前没有给出导出md的正式[API](https://www.wolai.com/wolai/7FB9PLeqZ1ni9FfD11WuUi)，所以是直接用了浏览器与后端直接交互接口
- 协作访客的工作区在导出时将得到空数据，因为这类工作区没有导出的权限

## 使用方法

https://github.com/tengfei-xy/wolai/releases
...
# 设定导出类型
# 大小写无关，可选值有html、md或markdown
exportType: ["html","md"]

# 对于家庭版、团队版、企业版子空间是必须的，需要填写此参数，subspace项可以填写多个
# 对于个人免费版、个人专业版，子空间是默认第一个的，无需（或任意）填写此参数
ignore:
    - workspace: 工作区A
      subspace:
        - name: 子空间A1
